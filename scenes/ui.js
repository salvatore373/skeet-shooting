...
let paused = false;
// Whether the sound is or or off
let muted = false;
...
loadSprite('sound_on', './sprites/sound_on.png')
loadSprite('sound_off', './sprites/sound_off.png')
...
function buildGameBtn(spriteId, topRightPos, onClick) {
    let btn = add([
        sprite(spriteId, {
            width: UI_BTN_SIZE,
        }),
...
function buildUi() {
    ...
    buildGameBtn('pause',
        vec2(width() - UI_CONT_MARGIN, UI_CONT_MARGIN),
        (btn) => {
            paused = !paused;
            btn.changeSprite(paused ? 'play' : 'pause');
        });

    // Display the sound controller
    buildGameBtn(muted ? 'sound_off' : 'sound_on',
        vec2(width() - 2 * UI_CONT_MARGIN - UI_BTN_SIZE, UI_CONT_MARGIN),
        (btn) => {
            muted = !muted;
            btn.changeSprite(muted ? 'sound_off' : 'sound_on');
        }
    );
}
